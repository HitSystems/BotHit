...
function botGuardaFoto(msg,estat){
	var keyboard = [];
	var botToken = '1175028468:AAGzPS8EvKKfLcnjOZozFj493tzJ9uATxCA';
	...
	if(msg.data===undefined || msg.data.split(' ')[1].length==0){
		...
		conexion.recHit(estat[0].Valor, Sql).then(info => {
			...
			if(!linea==[]) {keyboard.push(linea);linea=[];}
			var sqlimg = `insert into archivo (id, fecha, nombre) values (1, getdate(), '${msg.photo[0].file_id}')`;
			
			https.get(`https://api.telegram.org/bot${Token}/getFile?file_id=${msg.photo[0].file_id}`, (resp) => {
				let data = '';
				resp.on('data', (chunk) => {
				  data += chunk;
				});
				resp.on('end', () => {
				  ...
				  // Al ser una peticion asincrona, hay que hacerla dentro de la otra peticion, ya que sino no podriamos acceder al file_path de la imagen
				  /*download(`https://api.telegram.org/file/bot${Token}/${data.result.file_path}`, `foto.png`, function(){
						console.log('done');
					});*/
				});
			  }).on("error", (err) => {
			...
			bot.sendPhoto(msg.chat.id, 'AgACAgQAAxkBAAIN617Gb2otqKttoOeoRhs8dbUboP4aAALrtDEbCoo5UkzDFEVHpH8mefj4Il0AAwEAAwIAA20AAxtVAgABGQQ').then(function(resultFromSendPhoto){
		...
		bot.sendMessage(msg.from.id,'Foto Guardada a la Botiga !!');
	

//  Hola Santi. el fichero esta en c:\hitbot i ae llama File hay que subir lo a la base de datos
// la base de datos se llama estat[0].Valor i la tienda es Botiga
// La tabla se llama 
//select * from archivo
//id	nombre	extension	descripcion	mime	archivo	fecha	propietario	tmp	down
//50DE6116-DFE2-4355-921A-1C6C88D6C3B2	TPV	JPG	Foto TPV de Bis. Casol�	image/pjpeg	0xFF7FF002BB400FFD9	2009-07-27 10:59:00.000	Carles_Bosch	0	0
